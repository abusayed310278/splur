...
class ContentController extends Controller
{
    ...
    public function HomeCategoryContent(Request $request, string $cat_name): \Illuminate\Http\JsonResponse
    {
        try {
            $category = Category::where('category_name', 'like', $cat_name)->first();
            ...
            // Set a default and maximum limit for pagination to prevent performance issues.
            $limit = (int) $request->query('limit', 5);
            $limit = ($limit > 0 && $limit <= 5) ? $limit : 5;
            ...
            $contents = Content::with(['category:id,category_name', 'subcategory:id,name'])
                ...
                ->paginate($limit)
                ->through(function ($content) {
                    // Optimized: Decode image2 field. It can be a JSON string or null.
                    $image2Array = is_string($content->image2) ? json_decode($content->image2, true) : ($content->image2 ?? []);
                    if (!is_array($image2Array)) {
                        $image2Array = [];
                    }

                    // Generate fully qualified URLs for image2.
                    $image2Urls = collect($image2Array)->map(function (string $img) {
                        if (Str::startsWith($img, ['http://', 'https://'])) {
                            return $img;
                        }
                        $cleaned = ltrim($img, '/');
                        return url('uploads/content/' . $cleaned);
                    })->toArray();

                    // Process tags: convert a comma-separated string into an array.
                    // This handles null or empty strings gracefully.
                    $tagsArray = $content->tags ? array_map('trim', explode(',', $content->tags)) : [];
                    $tagsArray = array_filter($tagsArray);  // Remove any empty values
